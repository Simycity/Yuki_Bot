    ...
    try {
        ...
        if (sourceType == 'buffer') {
            if (!Buffer.isBuffer(video))
                throw new Error("O tipo selecionado da mídia foi Buffer mas a mídia não é um Buffer.");
            fs.writeFileSync(inputVideoPath, video);
        }
        else if (sourceType == 'url') {
            if (typeof video != 'string')
                throw new Error("O tipo selecionado da mídia foi URL mas a mídia não é uma URL.");
