...
export async function audioTranscription(audioBuffer, { deepgram_secret_key }) {
    try {
...
export async function musicRecognition(mediaBuffer, { acr_host, acr_access_key, acr_access_secret }) {
    try {
        ...
        let artists = [];
        for (let artist of recognitionResponse.metadata.humming[0].artists)
