...
export async function igCommand(client, botInfo, message, group) {
    ...
    for await (let media of igInfo.media) {
        if (media.type == "image")
            await Whatsapp.replyFileFromUrl(client, message.chat_id, 'imageMessage', media.url, '', message.wa_message, { expiration: message.expiration });
        if (media.type == "video")
            await Whatsapp.replyFileFromUrl(client, message.chat_id, 'videoMessage', media.url, '', message.wa_message, { expiration: message.expiration, mimetype: 'video/mp4' });
    }
}
...
export async function tkCommand(client, botInfo, message, group) {
    ...
    if (!Array.isArray(tiktok.url)) {
        if (tiktok.type == 'image')
            await Whatsapp.replyFileFromUrl(client, message.chat_id, 'imageMessage', tiktok.url, '', message.wa_message, { expiration: message.expiration });
        if (tiktok.type == 'video')
            await Whatsapp.replyFileFromUrl(client, message.chat_id, 'videoMessage', tiktok.url, '', message.wa_message, { expiration: message.expiration, mimetype: 'video/mp4' });
    }
    else {
        for await (const url of tiktok.url) {
            if (tiktok.type == 'image')
                await Whatsapp.replyFileFromUrl(client, message.chat_id, 'imageMessage', url, '', message.wa_message, { expiration: message.expiration });
            if (tiktok.type == 'video')
                await Whatsapp.replyFileFromUrl(client, message.chat_id, 'videoMessage', url, '', message.wa_message, { expiration: message.expiration, mimetype: 'video/mp4' });
        }
    }
}
