...
import { buildText, messageErrorCommandUsage, timestampToDate } from "../../utils/general.utils.js";
...
export async function apiCommand(client, botInfo, message, group) {
    ...
    if (!message.text_command || !supportedServices.includes(args[0].toLowerCase().trim()))
        throw new Error(messageErrorCommandUsage(botInfo, message));
    const [serviceName] = args;
    let replyText;
    if (serviceName.trim() == 'deepgram') {
        if (args.length != 2)
            throw new Error(adminCommands.api.msgs.reply_deepgram_error);
        ...
        if (args.length != 4)
            throw new Error(adminCommands.api.msgs.reply_acrcloud_error);
...
export async function sairCommand(client, botInfo, message, group) {
    ...
    if (message.isGroupMsg && currentGroups[indexGroup].id == message.chat_id)
        await Whatsapp.sendText(client, message.sender, replyText);
    else
        await Whatsapp.replyText(client, message.chat_id, replyText, message.wa_message, { expiration: message.expiration });
}
export async function vergruposCommand(client, botInfo, message, group) {
    const groupController = new GroupController();
    const adminCommands = commandsAdmin(botInfo);
...
export async function linkgrupoCommand(client, botInfo, message, group) {
    ...
    if (!currentGroups[indexGroup].admins.includes(botInfo.host_number))
...
export async function veradminsCommand(client, botInfo, message, group) {
    const userController = new UserController();
    const adminCommands = commandsAdmin(botInfo);
    const generalMessages = getGeneralMessages(botInfo);
    if (!message.isBotOwner)
        throw new Error(generalMessages.permission.owner_bot_only);
...
export async function rmadminCommand(client, botInfo, message, group) {
    ...
    if ((ownerData === null || ownerData === void 0 ? void 0 : ownerData.id) == userData.id)
...
export async function entrargrupoCommand(client, botInfo, message, group) {
    ...
    if (!groupResponse)
        await Whatsapp.replyText(client, message.chat_id, adminCommands.entrargrupo.msgs.reply_pending, message.wa_message, { expiration: message.expiration });
...
export async function fotobotCommand(client, botInfo, message, group) {
    var _a, _b;
    const adminCommands = commandsAdmin(botInfo);
    if (message.type != 'imageMessage' && ((_a = message.quotedMessage) === null || _a === void 0 ? void 0 : _a.type) != 'imageMessage')
        throw new Error(messageErrorCommandUsage(botInfo, message));
...
export async function nomepackCommand(client, botInfo, message, group) {
    const botController = new BotController();
    const adminCommands = commandsAdmin(botInfo);
    if (!message.args.length)
        throw new Error(messageErrorCommandUsage(botInfo, message));
...
export async function nomeautorCommand(client, botInfo, message, group) {
    const botController = new BotController();
    const adminCommands = commandsAdmin(botInfo);
    if (!message.args.length)
        throw new Error(messageErrorCommandUsage(botInfo, message));
...
export async function prefixoCommand(client, botInfo, message, group) {
    ...
    if (!message.args.length)
        throw new Error(messageErrorCommandUsage(botInfo, message));
    if (!supportedPrefixes.includes(message.text_command))
...
export async function verusuarioCommand(client, botInfo, message, group) {
    const userController = new UserController();
    const adminCommands = commandsAdmin(botInfo);
    const generalMessages = getGeneralMessages(botInfo);
    let targetUserId;
    ...
    if (!userData)
        throw new Error(adminCommands.verusuario.msgs.error_user_not_found);
